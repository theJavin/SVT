// nvcc SVTAfib.cu -o svtafib -lglut -lm -lGLU -lGL
...
{	
	...
	{	
		...
		ActionPotentialSpeed[i] = 0.01; // 0.2
	...
	{	
		ActionPotentialSpeed[i] = 0.0; // 0.2
		ActionPotentialDuration[i] = MuscleLength[i]/ActionPotentialSpeed[i];
	}
...
int n_body()
{
	...
	while(time < STOP_TIME)
	{
		...
		if((220.0 - DT < time) && (time < 220.0 + DT))
		{
			//turnOnNodeMuscles(31);
		}
		
		if((230.0 - DT < time) && (time < 230.0 + DT))
		{
			//turnOnNodeMuscles(51);
		}
		
		if((240.0 - DT < time) && (time < 230.0 + DT))
		{
			//turnOnNodeMuscles(61);
		}
